# 이주석 (Lee Jooseok)(=commentLee)
 
---
 
## Experience
 
| 프로젝트 | 설명 | 비고 | 기간 | 사용 기술 |
|---|---|---|---|---|
| **[삼성전자 VD사업부 연계 프로젝트]** | VLM + RAG 기반 공정 이미지 분석 자동화. | 우수상 (2등) | 2026.02 ~ 04 | Python, VLM, RAG, LLM as a Judge |
| **[Re:meet](https://youtu.be/q-osTdBBtCA?list=LL)** | 그리운 분과 사진을 찍을 수 있는 실시간 온라인 포토부스 서비스.  | 우수상 (2등) | 2026.01 ~ 02 | Java, Spring, WebSocket, WebRTC, Redis |
| **[MINIBITE](https://www.youtube.com/watch?v=lykRT3MIipU)** | 위고비 사용자를 위한 식단 관리 및 커뮤니티 서비스. | - | 2025.12 | Java, Spring, RAG |
| **[SSAFY 14기 AI 챌린지](https://github.com/jjoo-mi-jeong-seok/AI-Challenge)** | 이미지 기반 질의응답(VQA) 모델 개발 대회. | 244팀 중 4위 [트러블슈팅](https://velog.io/@comment/A100-환경에서-VQA에-적합한-모델-찾기) | 2025.10 | Python, VLM |
| **[배달공유 커뮤니티 '가치']** | 배달 주문을 함께할 사람을 모집하는 데스크톱 커뮤니티 앱 | - | 2022.04 ~ 06 | C#, WPF |
| **[알고리즘 동아리 ALPS]** | 매주 알고리즘 학습 및 문제 풀이, 코딩테스트 대비 | - | 2021.03 ~ 12 | C++, JAVA |
---

## Education

* **삼성청년SW아카데미(SSAFY)** (2025.07 ~ 현재) - 자바 전공 트랙
* **전북대학교** 컴퓨터공학부 졸업 (2019.03 ~ 2025.02)
    * **GPA: 4.32 / 4.5**
* **세화고등학교** 졸업 (2015.03 ~ 2018.02)

---

## Awards & Certification

* **삼성전자** - 삼성 SW 역량 테스트 **Pro 등급(B형)** 취득
* **삼성전자** - 삼성전자 VD·네트워크사업부 SSAFY 연계 프로젝트 **우수상** (2등)
* **SSAFY** - 공통 프로젝트 **우수상** (2등)
* **SSAFY** - 1학기 **성적우수상** (1등)
* **정보처리기사** - 한국산업인력공단 (2023)
* **TOEIC Speaking** - IM2 (2025)

---

## Links

* **Blog:** [Velog (@comment)](https://velog.io/@comment)
* **Email:** 1231js@jbnu.ac.kr
* [![Solved.ac프로필](http://mazassumnida.wtf/api/mini/generate_badge?boj=1231js)](https://solved.ac/1231js)
